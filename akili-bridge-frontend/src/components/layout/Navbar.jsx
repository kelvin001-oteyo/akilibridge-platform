...
export default function Navbar() {
  ...
  const dropdownItems = [
    ...
    { name: "My Applications", path: "/dashboard" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden lg:flex items-center gap-1">
            ...
            {/* Admin Dashboard Link - Hidden from public */}
            {isAuthenticated() && isAdmin() && (
              <Link
                to="/admin/dashboard"
                className="px-4 py-2 rounded-lg text-sm font-medium text-[#2fb3ff] bg-[#2fb3ff]/10 hover:bg-[#2fb3ff]/20 transition-all"
              >
                Dashboard
              </Link>
            )}
